...
var initHeader = (header) => {
  var menuBtn = header.querySelector(".header_v_1_art6__menu_btn"),
    ...
    mobileWrappers = Array.from(header.querySelectorAll(".header_v_1_art6__mobile_wrapper")),
    mobileCallBackButton = header.querySelector(".header_v_1_art6__mobile_phone");
  ...
  var mobileMenuTransition = (mobileButtons) => {
    ...
    mobileButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        var currentWrapper = btn.closest(".header_v_1_art6__mobile_wrapper");
...
var checkErorrs = (formElements) => {
  var isErrorConsist = formElements.some((el) => el.classList.contains("error"));
...
var initNashiRabotySliders = (container) => {
  ...
  var previewSliderEl = container.querySelector(".nashi-raboty-v1__thumb_slider");
...
var initVKVideo = (videos) => {
  ...
  videos.forEach((el) => {
    ...
    var videoFull = videoHref.substring(deletedLength, videoHref.length).split("_");
...
var initKomandaSlider = (sliderEl) => {
  var slider = null;
  ...
  var initSlider = () => {
    slider = new Swiper(sliderEl, {
      ...
      spaceBetween: 10,
      navigation: {
        prevEl: ".komanda_v_1__prev",
        nextEl: ".komanda_v_1__next",
      },
    });
  };

  window.innerWidth < 768 && initSlider();
  ...
  mm767.addEventListener("change", () => (mm767.matches ? initSlider() : disableSlider()));
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  var nashiRabotySlidersContainers = document.querySelectorAll(".nashi-raboty-v1__sliders");

  nashiRabotySlidersContainers.length > 0 && nashiRabotySlidersContainers.forEach((container) => initNashiRabotySliders(container));
  ...
  var free_width_sliders = Array.from(document.querySelectorAll(".js-free-width-slider"));

  free_width_sliders.length > 0 && free_width_sliders.forEach((slider) => initFreeWidthSlider(slider));
  ...
  var lidforma_zakrytaya_modal = document.querySelector(".js-lidforma-zakrytaya-modal");

  lidforma_zakrytaya_modal && initLidformaZakrytayaModal(lidforma_zakrytaya_modal);
  ...
  var tekst_s_foto_slider = Array.from(document.querySelectorAll(".js-tekst-foto-slider"));

  tekst_s_foto_slider.length > 0 && tekst_s_foto_slider.forEach((slider) => initTextSFotoSlider(slider));

  var komanda_sliders = Array.from(document.querySelectorAll(".js-komanda-slider"));
  komanda_sliders.length > 0 && komanda_sliders.forEach((slider) => initKomandaSlider(slider));
});
